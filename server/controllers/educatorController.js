...
export const updateRoleToEducator = async (req, res) => {
  try {
    ...
    // get logged in user (who is requesting role change)
    ...
    if (requestingUser.publicMetadata?.role !== "admin") {
      return res.status(403).json({
        success: false,
        message: "Only admins can assign educator role",
      });
    }

    // get target userId from request body (the one you want to promote)
    ...
    res.json({
      success: true,
      message: "User promoted to educator successfully",
    });
  } catch (error) {
